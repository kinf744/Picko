...
// Moteur hev commun Zamois-tun / Picko : libhev-socks5-tunnel.so (armeabi-v7a 32-bit).
// Fichier aligné sur Zamois-tun app/src/main/cpp/hev_jni.cpp, avec gardes Picko
// (config vide, échecs malloc/pthread) conservées.
...
Java_hev_htproxy_TProxyService_TProxyStartService(
    ...
    long sz = ftell(f);
    rewind(f);
    if (sz <= 0) { fclose(f); LOGE("Empty relay configuration"); return; }
    char* buf = (char*)malloc(sz + 1);
    if (!buf) { fclose(f); LOGE("Unable to allocate relay configuration"); return; }
    size_t rd = fread(buf, 1, (size_t)sz, f);
    buf[rd] = '\0';
    fclose(f);

    TunnelArgs* args = (TunnelArgs*)malloc(sizeof(TunnelArgs));
    if (!args) { free(buf); LOGE("Unable to allocate relay arguments"); return; }
    args->config = buf;
    args->fd     = (int)fd;
    ...
    if (pthread_create(&g_thread, nullptr, tunnel_thread, args) != 0) {
        ...
        free(buf);
        free(args);
        LOGE("Unable to start relay thread");
        return;
    }
