...
export default function DiagnosticScreen() {
  ...
  const filters: { key: "all" | LogLevel; label: string }[] = [
    ...
    { key: "error", label: t("Erreurs") },
    { key: "warning", label: t("Avertissements") },
    { key: "connection", label: t("Connexion") },
  ];
  ...
  const shareReport = async () => {
    await Share.share({ title: "KIGHMU VPN", message: `${t("Rapport KIGHMU VPN")}\n\n${report}` });
  };

  return <ScreenContainer className="px-5 pt-4" edges={["top", "left", "right", "bottom"]} swipeTabs>
    <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      <View style={styles.header}><View><Text className="text-3xl font-bold text-foreground">{t("Diagnostic")}</Text><Text className="mt-2 text-sm text-muted">{t("Les secrets sont filtrés avant affichage et partage.")}</Text></View><Pressable onPress={shareReport} style={({ pressed }) => [styles.iconButton, { borderColor: colors.border }, pressed && styles.pressed]}><Text style={{ color: colors.primary, fontSize: 13, fontWeight: "700" }}>{t("Partager")}</Text></Pressable></View>
