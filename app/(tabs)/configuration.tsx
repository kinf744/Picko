...
export default function ConfigurationScreen() {
  ...
  const profileEditor = () => {
    ...
    return <Panel raised style={styles.editorPanel}>
      ...
      {draft.kind === "zivpn" ? <><GroupTitle title="Accès UDP-ZIVPN" icon="shield" />{field("Host ou adresse IP", "host", "vpn.exemple.com ou 203.0.113.10")}{field("Port ou plage de ports", "port", "6000-19999", { keyboardType: "numeric" })}<GroupTitle title="Authentification" icon="key" />{field("Mot de passe", "password", "Mot de passe du serveur", { note: "Affiché à votre demande ; conservez cet écran à l’abri des regards." })}<GroupTitle title="Négociation de débit" icon="insights" />{field("Débit montant (Mbps)", "uploadMbps", "10", { keyboardType: "numeric", note: "Valeur envoyée au moteur UDP-ZIVPN pour la négociation montante." })}{field("Débit descendant (Mbps)", "downloadMbps", "50", { keyboardType: "numeric", note: "Valeur envoyée au moteur UDP-ZIVPN pour la négociation descendante." })}</> : null}
      ...
      {draft.kind === "hysteria" ? <><GroupTitle title="Connexion UDP" icon="speed" />{field("Host ou adresse IP", "host", "hysteria.exemple.com")}{field("Port ou plage UDP", "port", "443 ou 20000-50000", { keyboardType: "numeric" })}<GroupTitle title="Authentification" icon="key" />{field("Authentification Hysteria", "auth", "Mot de passe d’authentification", { note: "Affiché à votre demande ; conservez cet écran à l’abri des regards." })}{field("Obfs Hysteria", "obfs", "Clé Obfs", { note: "Toujours visible pour vérifier la valeur saisie ; conservez cet écran à l’abri des regards." })}<GroupTitle title="Débits du téléphone" icon="insights" />{field("Débit montant (Mbps)", "uploadMbps", "10", { keyboardType: "numeric", note: "Utilisez le débit montant réellement mesuré sur ce téléphone." })}{field("Débit descendant (Mbps)", "downloadMbps", "50", { keyboardType: "numeric", note: "Une valeur réaliste évite de limiter ou dégrader le tunnel." })}</> : null}
