...
export default function ConfigurationScreen() {
  ...
  const profileEditor = () => {
    ...
    return <Panel raised style={styles.editorPanel}>
      ...
      {draft.kind === "zivpn" ? <><GroupTitle title="Accès UDP-ZIVPN" icon="shield" />{field("Host ou adresse IP", "host", "vpn.exemple.com ou 203.0.113.10")}{field("Port ou plage de ports", "port", "6000-19999", { keyboardType: "numeric" })}<GroupTitle title="Authentification" icon="key" />{field("Obfs", "obfs", "Clé Obfs", { secureTextEntry: true })}{field("Mot de passe", "password", "Mot de passe du serveur", { secureTextEntry: true })}</> : null}
      {draft.kind === "slowdns" ? <><GroupTitle title="Transport DNS" icon="dns" />{field("Serveur DNS/UDP", "dnsServer", "203.0.113.10")}{field("Port DNS UDP", "dnsPort", "53", { keyboardType: "numeric" })}{field("Nameserver SlowDNS", "nameserver", "t.exemple.com")}{field("Clé publique dnstt", "publicKey", "Clé publique du serveur dnstt", { multiline: true })}<GroupTitle title="Accès SSH" icon="vpn-key" />{field("Hôte SSH attendu", "sshHost", "ssh.exemple.com", { note: "Libellé de contrôle : la cible SSH réelle est définie par le serveur SlowDNS." })}{field("Identifiant SSH", "sshUsername", "utilisateur SSH")}{field("Mot de passe SSH", "sshPassword", "Mot de passe SSH", { secureTextEntry: true })}</> : null}
      {draft.kind === "hysteria" ? <><GroupTitle title="Connexion UDP" icon="speed" />{field("Host ou adresse IP", "host", "hysteria.exemple.com")}{field("Port ou plage UDP", "port", "443 ou 20000-50000", { keyboardType: "numeric" })}<GroupTitle title="Authentification" icon="key" />{field("Authentification Hysteria", "auth", "Mot de passe d’authentification", { secureTextEntry: true })}{field("Obfs facultatif", "obfs", "Clé Obfs", { secureTextEntry: true })}<GroupTitle title="Débits du téléphone" icon="insights" />{field("Débit montant (Mbps)", "uploadMbps", "10", { keyboardType: "numeric", note: "Utilisez le débit montant réellement mesuré sur ce téléphone." })}{field("Débit descendant (Mbps)", "downloadMbps", "50", { keyboardType: "numeric", note: "Une valeur réaliste évite de limiter ou dégrader le tunnel." })}</> : null}
      ...
      {draft.kind === "v2ray-slowdns" ? <><GroupTitle title="Transport SlowDNS" icon="dns" />{field("Serveur DNS/UDP", "dnsServer", "203.0.113.10")}{field("Port DNS UDP", "dnsPort", "53", { keyboardType: "numeric" })}{field("Nameserver SlowDNS", "nameserver", "t.exemple.com")}{field("Clé publique dnstt", "publicKey", "Clé publique du serveur dnstt", { multiline: true })}<GroupTitle title="Configuration V2Ray" icon="hub" />{field("Configuration V2Ray", "json", "{ \"inbounds\": [], \"outbounds\": [] }", { multiline: true, note: "La configuration est utilisée à travers le transport SlowDNS." })}</> : null}
